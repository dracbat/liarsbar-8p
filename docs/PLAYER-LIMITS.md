...
**`GiveTurnSpinDeadSpinMoment()`** — the handover after a dead spin kills somebody in
Liar's Spin. Its `GiveTurnSpin` sibling was found and rewritten early because it uses the
plain `cmp ebx,3` shape; this one writes the wrap as a mask instead —
...
— so the scanner walked straight past it, and it shipped unpatched until v1.0.0. It only
...
**`ChaosDeckGameplay.GetAimTargetSlot(aim)`** — which seat an aim points at. A hand-written
table of four seats by three directions:

| my slot | aim −1 | aim 0 | aim +1 |
|---|---|---|---|
| 0 | 3 | 2 | 1 |
| 1 | 0 | 3 | 2 |
| 2 | 1 | 0 | 3 |
...
hard to hit — it is unreachable. A chaos card at eight players was never the mechanic it
...
`target = (mySlot + (2 - aim)) mod 4` — aim +1 is the next seat round, 0 is straight across,
−1 the seat before. That generalises on its own to

```
target = (mySlot + (n/2 - aim)) mod n,   aim from n/2-(n-1) up to n/2-1
```

At four players `n/2` is 2 and the range is −1..1: the same three answers for the same three
inputs. The replacement is the shipped rule with the four taken out of it, not a new rule
that happens to agree at four.

`GetAimDirectionForSlot` — the inverse, asked when the game forces somebody to shoot a
particular player — searches a hard-coded three-element list of aim values, so it needed
